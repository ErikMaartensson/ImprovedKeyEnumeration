...
Run like this::
    ...
    sage: attach("estimates_ISIT_version.py")
...
def getQuantumEnumerationCost(p, freq, N):
    enumerationList = [exactNumberOfEnumerationQuantum(p, freq, k) for k in range(N + 1)]
    x = list(range(N + 1))
    fit = exponentialApproximation(x, enumerationList)
    return enumerationList, fit

def getClassicalEnumerationCost(p, freq, N):
    enumerationList = [exactNumberOfEnumeration(p, freq, k) for k in range(N + 1)]
    x = list(range(N + 1))
    fit = exponentialApproximation(x, enumerationList)
    return enumerationList, fit
...
def runall(
    ...
    for scheme in schemes:
        ...
        prob = scheme[1]
        freq = scheme[2]
        enumCost, fit = getClassicalEnumerationCost(prob, freq, 40)
        enumCostQ, fitQ = getQuantumEnumerationCost(prob, freq, 40)
...
def results_table(results, fmt=None):
    ...
    def pp(cost):
        return round(math.log(cost["rop"], 2), 1)
